...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, name_labels, prefix_labels, star_nums, is_food) in enumerate(dataloader):
            ...
            # 前向传播
            embeddings, prefix_logits, star_logits, is_food_logits = model(images)
            ...
            if is_food.any():
                prefix_loss = prefix_loss_fn(prefix_logits[is_food], prefix_labels[is_food])
            else:
                prefix_loss = torch.tensor(0.0)
            star_loss = star_loss_fn(star_logits, star_nums)
            is_food_loss = is_food_loss_fn(is_food_logits, is_food.float())
    ...
    with torch.no_grad():
        ...
        torch.onnx.export(model, dummy_input, "model.onnx", input_names=['input_image'], output_names=['class_prototypes', 'prefix_logits', 'star_logits', 'is_food_logits'], dynamic_axes={'input_image' : {0 : 'batch_size'}})
        import onnx
